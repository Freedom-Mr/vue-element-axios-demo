<template>
  <div>
    <el-tooltip effect="light" content="全屏" placement="bottom">
      <!-- <svg t="1508738709248" @click='click' class="锁" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2069" xmlns:xlink="http://www.w3.org/1999/xlink" width="19" height="19">
  ...
  methods: {
    click () {
      this.$confirm('执行全屏操作', {
        ...
        confirmButtonText: this.fulltext,
        cancelButtonText: '部分全屏',
        type: 'warning'
      }).then(() => {
        ...
        this.fulltext = this.fulltext === '整体全屏' ? '取消全屏' : '整体全屏'
        screenfull.toggle()
      }).catch(action => {
        if (action === 'cancel') {
          this.fulltext = '整体全屏'
          var full = document.getElementById('app-main')
