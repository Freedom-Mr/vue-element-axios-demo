...
export default{
  ...
  mapToMenuPermissionTree (routeArr, parentPermVal) {
    return routeArr.map(route => {
      let obj = {}
      if (route.meta && route.meta.perm) {
        obj.val = route.meta.perm
        obj.name = route.meta.title
        obj.type = permType.MENU
        obj.parent = parentPermVal
        obj.hidden = route.hidden
        obj.sys = route.sys
      }
      if (route.children) {
        obj.children = this.mapToMenuPermissionTree(route.children, obj.val)
      }
      return obj
    })
  },
  ...
  mapToButtonPermissionTree (btnPermMap, menuPermissionTree) {
    return menuPermissionTree.map(perm => {
      if (perm) {
        ...
        if (perm.type === permType.MENU) {
          let btnPerms = btnPermMap[perm.val]
          if (btnPerms) {
            btnPerms.forEach(p => {
